...
RSpec.describe 'the bakery edit page', type: :feature do
  before :each do
  ...
  context 'index page link' do
    it 'has a link to the bakery index page' do
      visit "/bakeries/#{@bakery_1.id}/edit"
  ...
  context 'update form' do
    it 'has a form to update the bakery' do
      visit "/bakeries/#{@bakery_1.id}/edit"

      expect(page).to have_content("Fill out the fields below to update your Radical Bakery")

      fill_in 'Name', with: 'The Sweet Spot'
      select 'false', from: 'Open on weekends'
      fill_in 'Hours open', with: 6
